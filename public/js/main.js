...
function frame_window_init(){
	if(project_info.id.length==0){
		...
		$('#app_canvas_frame').attr('src','project_list');
...
function nav_init(){
	...
	$("#nav_project_add").click(function(){
		$('#app_canvas_frame').css({'height':'auto'});
		$('#frame_window').css({'width':600,'margin-top':-240,'margin-left':-300});
		$('#app_canvas_frame').attr('src','project_add');
		$('.frame_window_area').show();
	});
	...
	$("#nav_help_about").click(function(){
		$('#app_canvas_frame').css({'height':'auto'});
		$('#frame_window').css({'width':600,'margin-top':-240,'margin-left':-300});
		$('#app_canvas_frame').attr('src','about');
		$('.frame_window_area').show();
	});
	$("#nav_help_help").click(function(){
		$('#app_canvas_frame').css({'height':'auto'});
		$('#frame_window').css({'width':600,'margin-top':-240,'margin-left':-300});
		$('#app_canvas_frame').attr('src','help');
		$('.frame_window_area').show();
	});
...
function show_canvas(){
	...
	if(canvas_data.background_image!=""){
		var img_src="project/"+project_info.id+"/"+canvas_data.background_image;
...
function get_style(style){
	...
	if(style_obj['background_image']!=undefined){
		var background_css="background-image:url(project/"+project_info.id+"/"+style_obj['background_image']+");";
		ret+=background_css;
	}
...
function lib_show(){
	...
	$.ajax({
		url:imagepath + 'showlist',type:'POST',data:data_str,dataType:'json',timeout: ajax_timeout,error:function () {alert("加载文件列表失败！");},
		success:function(feedback){
			if(feedback.ret=="0"){
				var img_list=feedback.img_list;
				var img_html="";
				for(var i=0;i<img_list.length;i++){
					img_html+="<div class=\"item\" title=\""+img_list[i].name+"\" draggable=\"true\" data='"+$.toJSON(img_list[i])+"' style=\"height:"+img_list[i].thumb_height+"px;width:"+img_list[i].thumb_width+"px;background:url(/project/"+project_info.id+"/"+img_list[i].thumb_name+") no-repeat center top;\">"
					img_html+="</div>"
				}
				$("#lib_list").hide();
			...
			imageReader.onload = (function(aFile) {
				return function(e) {
					...
					if(img_data.length==files.length){
						...
						var data_str=requestToString(data_area);
						$.ajax({
							url:imagepath + 'upload',type:'POST',data:data_str,dataType:'json',timeout: ajax_timeout,error:function () {alert("上传文件失败！");},
							success:function(feedback){
								lib_show();
...
function canvas_bg_init(){
	...
	$("#canvas_bg_set")[0].ondrop = function(ev) {/*拖拽元素进入目标元素头上，同时鼠标松开的时候*/
		...
		var img_src="project/"+project_info.id+"/"+data.name;
...
function rabbish_init(){
	...
	$("#rabbish_area")[0].ondrop = function(ev) {/*拖拽元素进入目标元素头上，同时鼠标松开的时候*/
		...
		var data_str=requestToString(data_area);
		$.ajax({
			url:imagepath + 'del',type:'POST',data:data_str,dataType:'json',timeout: ajax_timeout,error:function () {alert("删除文件失败！");},
			success:function(feedback){
				if(feedback.ret=="0"){
					lib_show();
				}
...
var project={
	read:function(id){
		...
		var data_str=requestToString(data_area);
		$.ajax({
			url:cgipath + 'read',type:'POST',data:data_str,dataType:'json',timeout: ajax_timeout,error:function () {alert("保存失败！");},
			success:function(feedback){
				if(feedback.ret=="0"){
	...
	save:function(){
		...
		var data_str=requestToString(data_area);
		$.ajax({
			url:cgipath + 'save',type:'POST',data:data_str,dataType:'json',timeout: ajax_timeout,error:function () {alert("保存失败！");},
			success:function(feedback){
				if(feedback.ret=="0"){
